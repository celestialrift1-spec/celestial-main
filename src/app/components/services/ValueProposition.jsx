...
>
  <div className="max-w-7xl mx-auto">
    ...
    <div className="text-center mb-12 sm:mb-16 animate-fade-in-down">
      ...
      >
        <h2
          className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold text-[#27391C] tracking-tight leading-snug sm:leading-tight mb-4 sm:mb-5"
        >
        ...
        <p className="mt-3 sm:mt-5 text-sm sm:text-base text-gray-600 leading-relaxed max-w-xl sm:max-w-2xl mx-auto">
          Experience the difference of innovative solutions and trusted
          partnerships.
        </p>

        <motion.div
          className="w-16 h-1.5 sm:w-24 mx-auto bg-gradient-to-r from-[#B0DB9C] to-[#27391C] mt-6 sm:mt-8 rounded-full"
          initial={disableAnim || reduceMotion ? false : { width: 0 }}
          whileInView={disableAnim || reduceMotion ? false : { width: 64 }}
    ...
    {/* ✅ Value Grid */}
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-x-4 sm:gap-x-6 md:gap-x-8 gap-y-8 sm:gap-y-10 lg:gap-y-12">
      {values.map((value, index) => (
        ...
        >
          <div className="relative shadow-2xl transition-all duration-700 ease-out z-10 circle-card-container">
            <div className="p-6 sm:p-8 text-center transition-opacity duration-300 z-20">
              <h3
              ...
              <p
                className="text-xs sm:text-sm opacity-0 max-h-0 group-hover:opacity-100 group-hover:max-h-full transition-all duration-500 ease-in-out"
                style={{ color: "#E5E7EB" }}
              >
                {value.description}
                <span
