...
const LeadershipTeamSection = () => {
  ...
  return (
    <section className="relative bg-[#F8FAFC] font-sans py-10 sm:py-12 px-4 sm:px-6 lg:px-10 overflow-hidden">
  
  <div className="absolute inset-0 z-0 opacity-10">
    <div className="absolute top-1/4 left-[5%] w-32 h-32 sm:w-40 sm:h-40 bg-[#B0DB9C] rounded-full blur-2xl mix-blend-multiply"></div>
    <div className="absolute bottom-1/4 right-[5%] w-32 h-32 sm:w-40 sm:h-40 bg-[#27391C] rounded-full blur-2xl mix-blend-multiply"></div>
  </div>
  ...
  <div className="max-w-6xl mx-auto relative z-10">
    ...
    <div className="text-center mb-8 sm:mb-12">
      <div className="flex items-center justify-center gap-2 mb-2 sm:mb-3">
        <div className="h-px w-6 sm:w-8 bg-[#64748B]"></div>
        <FocusIcon className="w-4 h-4 sm:w-5 sm:h-5 text-[#27391C]" />
        <span className="text-[9px] xs:text-xs font-semibold uppercase tracking-widest text-[#27391C]">
          Our Leadership
        </span>
        <div className="h-px w-6 sm:w-8 bg-[#64748B]"></div>
      </div>
      <h2 className="text-2xl xs:text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#27391C] mb-2 sm:mb-3 leading-tight">
      ...
      <p className="text-sm xs:text-base sm:text-lg text-gray-600 leading-relaxed max-w-xl mx-auto">
    ...
    <div className="space-y-8 sm:space-y-12">
      {teamData.map((member, index) => (
        <div
          key={index}
          className={`flex flex-col rounded-2xl overflow-hidden shadow-xl transition-all duration-500 bg-white ${activeCard === index ? 'shadow-[#B0DB9C]/30' : 'shadow-gray-200'}`}
          onMouseEnter={() => setActiveCard(index)}
          onMouseLeave={() => setActiveCard(null)}
        >
          <div className={`flex flex-col ${index % 2 === 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'} items-center gap-4 sm:gap-6`}>
            
            {/* Image */}
            <div className={`w-full sm:w-5/12 lg:w-4/12 p-4 relative group ${member.colorText}`}>
            ...
            {/* Info */}
            <div className="w-full sm:w-7/12 lg:w-8/12 p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-5">
              <div>
                <h3 className="text-xl xs:text-2xl sm:text-3xl font-bold text-[#171717] mb-1">{member.name}</h3>
                <p className={`text-base xs:text-lg sm:text-xl font-medium mb-2 ${member.colorText}`}>{member.role}</p>
                <div className={`h-1 w-8 xs:w-10 ${member.colorGradient} bg-gradient-to-r rounded-full`}></div>
              </div>

              <div className="p-2 sm:p-3 bg-[#F8FAFC] border-l-4 border-[#B0DB9C] rounded-lg shadow-sm">
                <p className="text-sm xs:text-base italic text-[#64748B] leading-relaxed">
                  "{member.tagline}"
                </p>
              </div>

              <div className="space-y-1 sm:space-y-2">
                <h4 className="text-sm xs:text-base font-bold text-[#171717] border-b pb-1 border-gray-100">Key Achievements:</h4>
                {member.highlights.map((highlight, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <CheckCircleIcon className={`flex-shrink-0 w-3.5 h-3.5 sm:w-4 sm:h-4 mt-0.5 ${member.colorText}`} />
                    <p className="text-sm xs:text-base text-[#64748B]">{highlight}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {member.expertise.map((skill, i) => (
                  <span
                    key={i}
                    className={`px-3 sm:px-4 py-1.5 text-[9px] xs:text-xs font-semibold rounded-full border ${member.colorText} border-[#B0DB9C]/30 bg-white shadow-sm hover:bg-[#27391C] hover:text-white transition-all`}
                  >
                    {skill}
                  </span>
                ))}
